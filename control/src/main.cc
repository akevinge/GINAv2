...
void setup_lora_tasks() {
  ...
  sensor_queue =
      xQueueCreate(50, sizeof(sensor_data_t));  // Queue to hold sensor data

  configure_lora();
  xTaskCreatePinnedToCore(poll_sensor_task, "Poll_Sensor_Task", 4096,
                          (void*)sensor_queue, 1, NULL, tskNO_AFFINITY);
  xTaskCreatePinnedToCore(away_tx_task, "LoRa_TX_Task", 8192,
                          (void*)sensor_queue, 5, NULL, tskNO_AFFINITY);
  ...
  configure_lora();
  xTaskCreatePinnedToCore(away_rx_task, "Away_RX_Task", 8192,
                          (void*)command_queue, 5, NULL, tskNO_AFFINITY);
  xTaskCreatePinnedToCore(command_exe_task, "Command_Exe_Task", 8192,
                          (void*)command_queue, 5, NULL, tskNO_AFFINITY);
