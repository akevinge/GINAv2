...
void away_tx_task(void* pvParameters) {
  ...
  const size_t max_packets_per_batch =
      (LORA_MAX_PAYLOAD_SIZE - batch_overhead) / sizeof(sensor_data_t);
  ESP_LOGI(TAG, "Max packets per batch: %d", (int)max_packets_per_batch);
  const size_t telemetry_size =
      batch_overhead + max_packets_per_batch * sizeof(sensor_data_t);
  uint8_t* telemetry_buffer =
      static_cast<uint8_t*>(pvPortMalloc(telemetry_size));

  telemetry_batch_t* telemetry =
      reinterpret_cast<telemetry_batch_t*>(telemetry_buffer);
  ...
  while (1) {
    ...
    while (packets_in_batch < max_packets_per_batch &&
           xQueueReceive(sensor_queue, &sensor_data, pdMS_TO_TICKS(10)) ==
               pdPASS) {
    ...
    if (packets_in_batch > 0) {
      const size_t batch_size =
          batch_overhead + packets_in_batch * sizeof(sensor_data_t);
      ESP_LOGI(TAG, "Prepared batch with %d packets, total size %d bytes",
               (int)packets_in_batch, (int)batch_size);

      totalSent += packets_in_batch;
      if (LoRaSend((uint8_t*)telemetry, batch_size, SX126x_TXMODE_SYNC) ==
          false) {
      ...
      ESP_LOGI(TAG, "Transmission cycle took %lu ms",
...
void home_rx_task(void* pvParameters) {
  ...
  const size_t max_packets_in_fixed_payload =
      (LORA_PAYLOAD_LENGTH > sizeof(telemetry_batch_t))
          ? (LORA_PAYLOAD_LENGTH - sizeof(telemetry_batch_t)) /
    ...
    if (recLen == LORA_PAYLOAD_LENGTH) {
      ...
      if (recieved->count > max_packets_in_fixed_payload) {
        ESP_LOGE(pcTaskGetName(NULL),
                 "Telemetry count %d exceeds max packets %d for fixed payload",
                 recieved->count, (int)max_packets_in_fixed_payload);
      } else {
        ESP_LOGI(TAG, "Received batch with %d packets (fixed payload)",
                 recieved->count);
        for (uint8_t i = 0; i < recieved->count; ++i) {
          ...
          if (home_sensor_queue != NULL) {
            ...
            ESP_LOGE(
                TAG,
                "Home sensor queue NULL, cannot forward received telemetry");
          }
        }
      }
      ...
      ESP_LOGI(pcTaskGetName(NULL),
               "Received packet of size %d bytes, RSSI: %d dBm, SNR: %d dB",
               recLen, rssi, snr);

    } else {
      // Ignore or log too-small packets
      ESP_LOGD(pcTaskGetName(NULL),
               "Received packet too small to be telemetry: %d bytes", recLen);
    }

    vTaskDelayUntil(&xLastWakeTime,
                    pdMS_TO_TICKS(pdMS_TO_TICKS(1000 / LORA_TRANSFER_RATE_HZ)));
  }
...
void away_rx_task(void* pvParameters) {
  ...
  while (1) {
    ...
    if (recLen == LORA_PAYLOAD_LENGTH) {
      ...
      ESP_LOGI(pcTaskGetName(NULL), "Received command with action %d",
               recieved->action);
      if (command_queue != NULL) {
...
void lora_begin_config() {
  ...
  if (LoRaBegin(LORA_FREQUENCY_HZ, LORA_TX_POWER_DBM, LORA_TCXO_VOLTAGE,
                LORA_USE_REGULATOR_LDO) != 0) {
...
void configure_lora() {
  ...
  // LoRa transmission configuration - Minimum range + security and maximum rate
  // at 7, 6, 1, 8, 0

  LoRaConfig(LORA_SPREADING_FACTOR, LORA_BANDWIDTH, LORA_CODING_RATE,
             LORA_PREAMBLE_LENGTH, LORA_PAYLOAD_LENGTH, LORA_CRC_ON,
             LORA_INVERT_IRQ);
}